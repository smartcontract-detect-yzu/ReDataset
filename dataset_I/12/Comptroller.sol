    ...
    ExponentialNoError
{
    ...
    event NewBProtocol(
        ...
        address oldBProtocol,
        address newBProtocol
    );
    ...
    function liquidateBorrowAllowed(
        ...
        if (
            bLiquidator != address(0) &&
            IBProtocol(bLiquidator).canLiquidate(
                cTokenBorrowed,
                cTokenCollateral,
                repayAmount
            )
    ...
    function _disableMarket(CToken cToken) external returns (uint) {
        ...
        require(
            CToken(cToken).totalBorrowsCurrent() == 0,
            "market has borrows"
        );
