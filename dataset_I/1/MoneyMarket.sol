...
contract CarefulMath is ErrorReporter {
    ...
    function addThenSub(
        ...
        uint b,
        uint c
    ) internal pure returns (Error, uint) {
...
contract SafeToken is ErrorReporter {
    ...
    function doTransferIn(
        ...
        token.transferFrom(from, address(this), amount); // transfer amount from ‘from’ to ‘this’
    ...
    function mix_test(
        ...
        address ttto = to;
        token.transfer(ttto, amount);
        abcd.transfer(amount);
    }
    ...
    function doTransferOut(
        ...
        mix_test(token, msg.sender, to, amount);
...
contract Exponential is ErrorReporter, CarefulMath {
    ...
    function mulExp(
        ...
        (Error err1, uint doubleScaledProductWithHalfScale) = add(
            halfExpScale,
            doubleScaledProduct
        );
        if (err1 != Error.NO_ERROR) {
...
contract MoneyMarket is Exponential, SafeToken {
    ...
    event NewRiskParameters(
        uint oldCollateralRatioMantissa,
        uint newCollateralRatioMantissa,
        uint oldLiquidationDiscountMantissa,
        uint newLiquidationDiscountMantissa
    );
    ...
    event NewOriginationFee(
        uint oldOriginationFeeMantissa,
        uint newOriginationFeeMantissa
    );
    ...
    function calculateBalance(
        uint startingBalance,
        uint interestIndexStart,
        uint interestIndexEnd
    ) internal pure returns (Error, uint) {
        ...
        (Error err0, uint balanceTimesIndex) = mul(
            startingBalance,
            interestIndexEnd
        );
        if (err0 != Error.NO_ERROR) {
     ...
     */
    function calculateBorrowAmountWithFee(
        uint borrowAmount
    ...
    function _setRiskParameters(
        uint collateralRatioMantissa,
        uint liquidationDiscountMantissa
    ) public returns (uint) {
        // Check caller = admin
        if (msg.sender != admin) {
            return
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setOriginationFee(
        uint originationFeeMantissa
    ) public returns (uint) {
        // Check caller = admin
        if (msg.sender != admin) {
            return
    ...
    function _withdrawEquity(address asset, uint amount) public returns (uint) {
        ...
        (Error err0, uint equity) = addThenSub(
            cash,
    ...
    function supply(address asset, uint amount) public returns (uint) {
        ...
        Market storage market = markets[asset];
        Balance storage balance = supplyBalances[msg.sender][asset]; //First, we read the balance variable that represents the user’s deposited asset balance in MoneyMarket storage (line 1514)
        ...
        err = checkTransferIn(asset, msg.sender, amount); // This function (externally) calls the asset contract in order to figure if the user has the number of tokens he wishes to deposit and that he approved the MoneyMarket contract to withdraw this amount on his behalf.
        if (err != Error.NO_ERROR) {
        ...
        err = doTransferIn(asset, msg.sender, amount); //externally calls the asset contract’s transferFrom() function (line 405) that in turn transfers the amount from the user to the MoneyMarket contract.
        if (err != Error.NO_ERROR) {
            // This is safe since it's our first interaction and it didn't do anything if it failed
    ...
    function withdraw(
        ...
        address a_to_objective = msg.sender;
        ...
        // We ERC-20 transfer the asset into the protocol (note: pre-conditions already checked above)
        err = doTransferOut(asset, a_to_objective, localResults.withdrawAmount);
        if (err != Error.NO_ERROR) {
            // This is safe since it's our first interaction and it didn't do anything if it failed
     ...
     */
    function calculateAccountLiquidity(
        address userAddress
    ) internal view returns (Error, Exp memory, Exp memory) {
        Error err;
    ...
    function liquidateBorrow(
        ...
        Balance
            storage supplyBalance_LiquidatorCollateralAsset = supplyBalances[
    ...
    function calculateDiscountedBorrowDenominatedCollateral(
        ...
        uint supplyCurrent_TargetCollateralAsset
    ) internal view returns (Error, uint) {
