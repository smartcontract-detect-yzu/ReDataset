...
contract HypeBears is ERC721("HypeBears", "HB"), ERC721Enumerable, Ownable {
    ...
    function canMint(
        ...
        if (!whitelist[_address]) {
            bytes32 hash = keccak256(
                abi.encodePacked(whitelistSigner, _address)
            );
    ...
    function addPartnerMint(address account, uint256 amount) public onlyOwner {
        ...
        require(
            totalSupply().add(partnerMintAmount) <= TOTAL_NFT,
            "Can't add partner more than available"
        );
    ...
    function mintNFT(
        ...
        require(
            mintPrice.mul(_numOfTokens) <= msg.value,
            "Insufficient payable value"
        );
        require(
            totalSupply().add(_numOfTokens).add(partnerMintAmount) <= TOTAL_NFT,
            "Can't mint more than 10000"
        );

        (bool success, string memory reason) = canMint(msg.sender, _signature); // check the addressMinted
        require(success, reason);

        for (uint i = 0; i < _numOfTokens; i++) {
            _safeMint(msg.sender, totalSupply() + 1); // sink
        }
        addressMinted[msg.sender] = true; // source
    }
    ...
    function partnersMintMultiple(address[] memory _to) public {
        ...
        require(
            partnerMintAmount >= amount,
            "Can't mint more than total available for partners"
        );
        require(
            partnerMintAvailableBy[msg.sender] >= amount,
            "Can't mint more than available for msg.sender"
        );
