...
contract SavingsModule is Module, AccessChecker, RewardDistributions, CapperRole {
    ...
    returns(uint256) 
    {
        ...
        depositToProtocol(_protocol, _tokens, _dnAmounts);  //TAG: 外部调用, 存入当前合约
        ...
        if(nAmount > nDeposit) {
			...
			//['nextDistributionTimestamp', 'nextDistributions', 'distributionAccumulator', 
			//   'distributions', '_totalSupply', '_balances']
            poolToken.mint(_msgSender(), nDeposit); //TAG:铸造token
        } else {
            fee = 0;
            poolToken.mint(_msgSender(), nAmount); //TAG:铸造token
