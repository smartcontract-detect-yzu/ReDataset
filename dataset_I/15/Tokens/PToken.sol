...
abstract contract PToken is PTokenInterface, Exponential, TokenErrorReporter {
    ...
    function transferTokens(
        ...
        uint allowed = controller.transferAllowed(
            address(this),
            src,
            dst,
            tokens
        ...
        if (allowed != 0) {
            return
                failOpaque(
                    ...
                    FailureInfo.TRANSFER_CONTROLLER_REJECTION,
                    allowed
                );
        }
    ...
    function getBorrowSnapshot(
        address account
    ) external view returns (uint, uint) {
        return (
            accountBorrows[account].principal,
            accountBorrows[account].interestIndex
        );
    }
    ...
    function mintFresh(
        ...
        uint allowed = controller.mintAllowed(
            address(this),
            minter,
            mintAmount
        );
        if (allowed != 0) {
            return (
                failOpaque(
                    Error.CONTROLLER_REJECTION,
                    FailureInfo.MINT_CONTROLLER_REJECTION,
                    allowed
                ),
                0,
                0
    ...
    function redeemFresh(
        ...
        uint allowed = controller.redeemAllowed(
            address(this),
            redeemer,
            vars.redeemTokens
        );
        if (allowed != 0) {
            return (
                failOpaque(
                    Error.CONTROLLER_REJECTION,
                    FailureInfo.REDEEM_CONTROLLER_REJECTION,
                    allowed
                ),
                0,
                0
    ...
    function borrowFresh(
        ...
        uint allowed = controller.borrowAllowed(
            address(this),
            borrower,
            borrowAmount
        );
    ...
    function repayBorrowFresh(
        ...
        uint allowed = controller.repayBorrowAllowed(
            address(this),
            payer,
            borrower,
            repayAmount
    ...
    function liquidateBorrowFresh(
        ...
        (uint amountSeizeError, uint seizeTokens) = controller
        ...
        if (address(pTokenCollateral) == address(this)) {
            ...
            seizeError = pTokenCollateral.seize(
                liquidator,
                borrower,
                seizeTokens
            );
    ...
    function seizeInternal(
        ...
        uint allowed = controller.seizeAllowed(
            address(this),
            seizerToken,
            liquidator,
            borrower,
        ...
        if (allowed != 0) {
            return
                failOpaque(
                    Error.CONTROLLER_REJECTION,
                    FailureInfo.LIQUIDATE_SEIZE_CONTROLLER_REJECTION,
                    allowed
                );
        }
    ...
    function _setController(
        ...
        if (msg.sender != getMyAdmin()) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.SET_CONTROLLER_OWNER_CHECK
                );
        }
    ...
    function _setControllerInternal(
        ControllerInterface newController
    ) internal returns (uint) {
    ...
    function _setInterestRateModelFreshInternal(
        InterestRateModel newInterestRateModel
    ) internal returns (uint) {
