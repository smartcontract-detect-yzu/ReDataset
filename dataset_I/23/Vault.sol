    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (uint256 shares)
    {
        ...
        uint256 totalDeposit = ETHLeverage(controller).totalAssets();
        console.log("Current Total: ", totalDeposit);

        // Calls Deposit function on controller
        uint256 newDeposit = ETHLeverage(controller).deposit(assets);
        ...
        shares = totalSupply() == 0 || totalDeposit == 0
            ? assets
            : (totalSupply() * newDeposit) / totalDeposit;
    ...
    function withdraw(
        ...
        uint256 withdrawn = ETHLeverage(controller).withdraw(assets);
    ...
    function totalAssets() public view virtual returns (uint256) {
        return ETHLeverage(controller).totalAssets();
    }
