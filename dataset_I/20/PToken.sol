...
contract PToken is PTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function transferTokens(
        ...
        uint allowanceNew = sub_(
            startingAllowance,
            tokens,
            "ALLOWANCE_NOT_ENOUGH"
        );

        uint srcTokensNew = sub_(
            accountTokens[src],
            tokens,
            "BALANCE_NOT_ENOUGH"
        );
    ...
    function exchangeRateStoredInternal() internal view returns (uint) {
        ...
        if (_totalSupply == 0) {
            ...
            uint cashPlusBorrowsMinusReserves = sub_(
    ...
    function mintFresh(
        ...
        if (totalSupply == 0 && MINIMUM_LIQUIDITY > 0) {
            ...
            vars.mintTokens = sub_(
                vars.mintTokens,
                MINIMUM_LIQUIDITY,
                "FIRST_MINT_NOT_ENOUGH"
            );
    ...
    function _setProtocolSeizeShareMantissa(
        uint newProtocolSeizeShareMantissa
    ) external {
        onlyAdmin();

        emit NewProtocolSeizeShareMantissa(
